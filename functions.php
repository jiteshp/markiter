 ...
 * @return void
 */
function markiter_the_guide_title() {
	global $post;
	$guide_title = '';

	if ( has_post_parent() ) {
		$guide_title = sprintf(
			'<h4 class="guide-title">%s</h4>',
			apply_filters( 'the_title', get_the_title( $post->post_parent ) )
	 	);
	}

	the_title( '<header class="entry-header">' . $guide_title . '<h1 class="entry-title">', '</h1></header>' );
}

/**
 * Display the guide navigation menu.
 *
 * @return void
 */
function markiter_the_guide_nav_menu() {
	global $post;
	$topic_id = $post->ID;
	$result = '';
	...
	if ( $cluster_posts ) {
		$result .= '<nav class="guide-nav-menu" role="navigation">';

		if ( has_post_parent() ) {
			$result .= sprintf( '<h4><a href="%1$s">%2$s</a></h4>', esc_url( get_the_permalink( $topic_id ) ), apply_filters( 'the_title', get_the_title( $topic_id ) ) );
		} else {
			$result .= sprintf( '<h4>%s</h4>', apply_filters( 'the_title', get_the_title( $topic_id ) ) );
		}

		$result .= '<ol class="guide-pages">';

		foreach ( $cluster_posts as $cluster_post ) {
			$output = '<li class="guide-page-item"><a href="%2$s">%1$s</a></li>';
			if ( $post->ID == $cluster_post->ID ) {
				$output = '<li class="guide-page-item current-guide-page-item">%1$s</li>';
			}

			$result .= sprintf(
				$output,
				apply_filters( 'the_title', get_the_title( $cluster_post->ID ) ),
				get_the_permalink( $cluster_post->ID )
			);
		}
